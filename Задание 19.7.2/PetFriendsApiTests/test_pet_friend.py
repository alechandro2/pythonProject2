...
def test_add_pets_with_valid_data(name='Lewa', animal_type='Kotik', age='40', pet_photo='images/Lion.jpg'):
...
def test_delete_pet():
    '''Проверить возможность удаления питомца'''
    _, api_key = pf.get_app_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')

    if len(my_pets['pets']) == 0:
        pf.add_new_pets(api_key, 'Lewa', 'Kotik', '40', 'images/Lion.jpg')
        _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_update_pet_info(name='Misha', animal_type='Boxer', age='40'):
    '''Проверить возможность изменения данных питомца'''
    _, api_key = pf.get_app_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(api_key, 'my_pets')
...
def test_add_pets_with_valid_data_without_photo(name='No name no frame', animal_type='', age='25'):
    '''Проверить возможность добавления нового питомца без фото'''
    _, api_key = pf.get_app_key(valid_email, valid_password)
...
def test_add_photo_at_pet(pet_photo='images/horyok.jpg'):
    '''Проверить возможность добавления новой фотографии питомца'''
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
...
def test_add_pet_negative_age_number(name='Lewa', animal_type='Kotik', age='-99', pet_photo='images/Lion.jpg'):
    '''Негативный тест. Добавление питомца с отрицательным числом -99 в переменной age.
    Тест не будет пройден если питомец будет добавлен на сайт с отрицательным числом в поле ageт.
     '''
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)
...
def test_add_pet_with_four_digit_age_number(name='Lewa', animal_type='Kotik', age='9999', pet_photo='images/Lion.jpg'):
    '''Проверка с негативным сценарием. Добавление питомца с числом больше 3-х знаков в переменной age.
...
def test_add_pet_with_empty_value_in_variable_name(name='', animal_type='', age='40', pet_photo='images/Lion.jpg'):
    '''Проверить возможность добавление питомца с пустым значением в переменной name
...
def test_add_pet_with_a_lot_of_words_in_variable_name(animal_type='Kotik', age='40', pet_photo='images/Lion.jpg'):
    '''. Добавление питомца имя которого превышает 10 слов
    Тест не будет пройден если питомец будет добавлен на сайт с именем > 10 слов'''

    name = 'Sara Connor McGreggor Vasja Kot Batman Lion Murzik Tom Jerry Muska Buska'
    ...
    assert status == 200
    assert word_count < 10, 'Питомец добавлен с именем > 10 слов'


def test_add_pet_with_special_characters_in_variable_animal_type(name='Lewa', age='40', pet_photo='images/Lion.jpg'):
    '''Негативный тест. Добавление питомцав переменной animal_type со спецсимволами.
...
def test_add_pet_with_numbers_in_variable_animal_type(name='Lewa', animal_type='911', age='40',
                                                      pet_photo='images/Lion.jpg'):
    ...
    assert status == 200
    assert animal_type not in result['animal_type'], 'Питомец добавлен на сайт с цифрами вместо в поле порода'


def test_add_pet_with_a_lot_of_words_in_variable_animal_type(name='Lewa', age='2', pet_photo='images/Lion.jpg'):
    ...
    animal_type = 'Лев король Кот Чеширский Бульдог Французский Рысь европейская Рикки Тикки Тави Месси '
    ...
    assert status == 200
    assert word_count < 10, 'Происходит добавление питомца длиной больше 10-ти слов в названии животного '
