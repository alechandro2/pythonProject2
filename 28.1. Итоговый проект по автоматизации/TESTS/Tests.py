...
def test_autoriz_valid_email_pass(selenium):
    """EXP-001 Тест авторизации с валидными (правильными)значениями e-mail и паролем."""
...
def test_elements_of_autoriz(selenium):
    """EXP-004 Проверка Формы "Авторизация" на наличие основных элементов."""
    page = AuthPage(selenium)
...
def test_menu_type_autoriz(selenium):
    """EXP-005 Проверка названия табов в меню выбора типа авторизации."""
    try:
        page = AuthPage(selenium)
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    """EXP-016 Тест поля ввода "Имя" формы "Регистрация" допустимыми валидными значениями:
...
def test_first_name_invalid_data(selenium, invalid_first_name):
    """EXP-017 Тест поля ввода "Имя" формы "Регистрация" невалидными значениями:
    пустое значение;
...
def test_last_name_valid_data(selenium, valid_password):
    """EXP-018 Тест поля ввода "Пароль" формы "Регистрация" валидными значениями.:
...
def test_registration_confirm_password_valid_data(selenium):
    """EXP-019 Тест поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация»
...
def test_registration_confirm_password_invalid_data(selenium):
    """EXP-020 Тест поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация»
